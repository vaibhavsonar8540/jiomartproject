...
const GroceryDis = () => {
  ...
  useEffect(() => {
    axios.get(" http://localhost:3000/groceryitemdis",{params: paramObj }) 
  ...
  <option value="asc">Low to High</option>
        ...
        <h1 style={{marginLeft:"30px"}}>Grocery Discount Items</h1>
        <div className="product-grid">
          {data.map((el) => (
            <div className="product-card" key={el.id}>
              <span className="favorite-icon"><CiHeart onClick={() => handleFav(el)}/></span>
              <Link to={`/product/groceryitemdis/${el.id}`}>
